...
async function bootstrap() {
  console.log('🚀 Starting DLC API v1.2.2-alpha...');
  ...
  if (process.env.SWAGGER_ENABLED === 'true') {
    ...
    const config = new DocumentBuilder()
      .setTitle('DLC API')
      .setVersion(process.env.APP_VERSION ?? '1.2.2-alpha')
      .addBearerAuth()
      .build();
  ...
  fastifyInstance.get('/', async (_request, reply) => {
    reply.send({ message: 'DLC API Root - v1.2.2-alpha', status: 'running' });
  });
  ...
  console.log('');
  console.log('✅ DLC API v1.2.2-alpha running securely on port', env.apiPort);
